...
from src.app.schemas.graph import ReferenceEdge, ExternalLawNode, RefersToExternalEdge
...
class DeterministicExtractor(metaclass=SingletonMeta):
    ...
    def _parse_internal(self, source_id: str, content: str, result: DeterministicResult) -> None:
        ...
        for match in self.REGEX_INTERNAL.finditer(content):
            ...
            # 1. State array to hold whatever targets this regex match produces
            target_ids = []

            # 2. Structural Parsing (Only handles formatting)
            match valid_groups:
                # Format: "art. 11.2 - 11.4"
                case {"abs_art_range": _, "r_start": start, "r_end": end}:
                    target_ids.extend([
                        f"art_{start.replace('.', '_')}",
                        f"art_{end.replace('.', '_')}"
                    ])

                # Format: "art. 13 alin. (3)"
                case {"abs_art_alin": _, "aa_art": art, "aa_alin": alin}:
                    target_ids.append(f"art_{art.replace('.', '_')}_alin_{alin.replace('.', '_')}")

                # Format: "art. 102"
                case {"abs_art": _, "a_art": art}:
                    target_ids.append(f"art_{art.replace('.', '_')}")

                # Format: "alin. (3)" -> Requires Relative Context
                case {"rel_alin": _, "r_alin": alin}:
                    if base_article:
                        target_ids.append(f"{base_article}_alin_{alin.replace('.', '_')}")

                # Fallback for safe degradation
                case _:
                    logger.debug(f"Regex matched but fell through structural matching on ID {source_id}")

            # 3. Centralized Edge Creation (The single point of truth)
            for t_id in target_ids:
                if source_id != t_id:
                    result.internal_edges.append(
                        ReferenceEdge(source_id=source_id, target_id=t_id)
                    )
